...
ssh -q $CLIENT_HOST -- sudo docker run -v /tmp:/tmp \
    $BENCH_IMAGE \
    cp -r /optimal-bin/rw /tmp/

ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/rw/init create
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/rw/init populate
...
kubectl create configmap env-config \
    ...
    --from-literal=NUM_OPS=$NUM_OPS \
    --from-literal=READ_RATIO=$READ_RATIO \
