...
ssh -q $CLIENT_HOST -- sudo docker run -v /tmp:/tmp \
    $BENCH_IMAGE \
    cp -r /beldi-bin/bretwis /tmp/

ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX NUM_USERS=$NUM_USERS \
                       NUM_FOLLOWERS=$NUM_FOLLOWERS MAX_RETURN_POSTS=$MAX_RETURN_POSTS \
    /tmp/bretwis/init create
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX NUM_USERS=$NUM_USERS \
                       NUM_FOLLOWERS=$NUM_FOLLOWERS MAX_RETURN_POSTS=$MAX_RETURN_POSTS \
    /tmp/bretwis/init populate
...
scp -q $ROOT_DIR/workloads/workflow/beldi/benchmark/retwis/workload.lua $CLIENT_HOST:/tmp
