...
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/recovery/init create
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/recovery/init populate
...
ssh -q $MANAGER_HOST -- TABLE_PREFIX=$TABLE_PREFIX NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
                        NUM_OPS=$NUM_OPS READ_RATIO=$READ_RATIO LoggingMode=$LOGMODE FAIL_RATE=$FAIL_RATE \
    docker stack deploy \
...
scp -q $ROOT_DIR/workloads/workflow/optimal/benchmark/recovery/workload.lua $CLIENT_HOST:/tmp
