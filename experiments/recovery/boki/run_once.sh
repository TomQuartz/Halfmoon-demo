...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    $BENCH_IMAGE \
    cp -r /bokiflow-bin/recovery /tmp/
...
ssh -q $MANAGER_HOST -- TABLE_PREFIX=$TABLE_PREFIX NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
                        NUM_OPS=$NUM_OPS READ_RATIO=$READ_RATIO FAIL_RATE=$FAIL_RATE \
    docker stack deploy \
...
scp -q $ROOT_DIR/workloads/workflow/boki/benchmark/recovery/workload.lua $CLIENT_HOST:/tmp
