...
ssh -q $MANAGER_HOST -- TABLE_PREFIX=$TABLE_PREFIX NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
                        NUM_OPS=$NUM_OPS READ_RATIO=$READ_RATIO \
    docker stack deploy \
...
scp -q $ROOT_DIR/workloads/workflow/beldi/benchmark/rw/workload.lua $CLIENT_HOST:/tmp
...
ssh -q $CLIENT_HOST -- $WRK_DIR/wrk -t 2 -c 2 -d 300 -L -U \
    -s /tmp/workload.lua \
