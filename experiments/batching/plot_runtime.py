...
def plot(boki, hm_read, hm_write, hm_read_b, hm_write_b, read_ratios, figname):
    font_size = 30
    legend_size = 30
    legend_length = 2
    ...
    hm_read_b_curve = ax.plot(
        read_ratios,
        hm_read_b,
        linestyle="-",
        label="Halfmoon-read(batching)",
        marker="o",
        color="darkviolet",
        markersize=markersize,
        linewidth=linewidth,
    )
    hm_write_b_curve = ax.plot(
        read_ratios,
        hm_write_b,
        linestyle="-",
        label="Halfmoon-write(batching)",
        marker="d",
        color="deeppink",
        markersize=markersize,
        linewidth=linewidth,
...
if __name__ == "__main__":
    ...
    for qps in args.qps:
        ...
        for rr in args.read_ratios:
            ...
            hm_write.append(p50)
            p50, _, _ = summary("optimal-batch", exp_name, run, "write")
            hm_read_b.append(p50)
            p50, _, _ = summary("optimal-batch", exp_name, run, "read")
            hm_write_b.append(p50)
        plot(boki, hm_read, hm_write, hm_read_b, hm_write_b, args.read_ratios, f"{run}/batching_QPS{qps}.png")
