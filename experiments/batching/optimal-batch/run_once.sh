...
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/batching/init create
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION LoggingMode=$LOGMODE NUM_KEYS=$NUM_KEYS VALUE_SIZE=$VALUE_SIZE \
    /tmp/batching/init populate
...
scp -q $ROOT_DIR/workloads/workflow/optimal/benchmark/batching/workload.lua $CLIENT_HOST:/tmp
