...
for NODE in $NODES; do
  ...
  MEMORY_MI=$(kubectl top node $NODE --no-headers | awk '{print $4}' | sed 's/Mi//')
...
AVERAGE_MEMORY_MB=$(echo "scale=2; $AVERAGE_MEMORY_BYTES/1024/1024" | bc)

echo "Log memory usage: $AVERAGE_MEMORY_MB MB"
